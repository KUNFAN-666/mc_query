...
将本目录复制到 AstrBot 的 `data/plugins/mc_query/`，重启 AstrBot。
...
| `show_edition` | 是否显示 Java版/基岩版 标识 | 开 |
...
- 自动识别 **Java版 / 基岩版**：Java 状态查询失败后会自动用 RakNet 探测基岩服，并在版本下方显示。基岩版无真实 favicon，缺省用内置 `logo.png` 兜底。
- 查询时会做 DNS 重试，减少偶发 `gaierror` 报错。
- 支持 Minecraft **SRV 记录**：域名没有 A 记录时（如 `mc.cneko.org` 指向 `_minecraft._tcp` SRV）会自动解析到真实主机/端口。
